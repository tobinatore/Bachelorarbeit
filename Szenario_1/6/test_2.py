...
def sim_traffic(eid_send: str, proxy) -> None:
    ...
    with proxy.bp_open(eid_send) as eid:
        while True:
            ...
            dest_eid = "ipn:" + dest + ".2"
            eid.bp_send(dest_eid, b"HAM", report_eid="ipn:6.2")
...
if __name__ == "__main__":
    ...
    proxy = pyion.get_bp_proxy("6")
    proxy.bp_attach()

    l = threading.Thread(target=listen, args=["ipn:6.2", proxy])
    l.daemon = True
    l.start()

    t = threading.Thread(target=sim_traffic, args=["ipn:6.1", proxy])
    t.daemon = True
    t.start()
